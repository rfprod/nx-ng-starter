...
describe('client-util', () => {
  ...
  it('should throw error if a name has incorrect format', async () => {
    const expected = 'The name must start with client-util- and contain only lower case letters and dashes.';
    await expect(generator.default(tree, { ...context, name: 'util-client' })).rejects.toThrow(expected);
  });
  ...
  it(
    ...
    async () => {
      ...
      const testSetupIncludes = defaultTestSetupIncludes().filter(item => item.includes('setupJsdomGlobalMocks'));
